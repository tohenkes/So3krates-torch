...
class RealSphericalHarmonics(nn.Module):
    def __init__(self, l_max: int):
        super().__init__()
        assert 0 <= l_max <= 4, "This implementation supports l_max in [0, 4]"
        self.l_max = l_max
    ...
    def forward(self, vecs: torch.Tensor) -> torch.Tensor:
        ...
        if self.l_max >= 3:
            out += [
                0.25 * sqrt(35 / (2 * PI)) * y * (3 * x**2 - y**2),          # Y_3^-3
        ...
        if self.l_max >= 4:
            out += [
                ...
                0.75 * sqrt(35 / (2 * PI)) * y * (3 * x**2 - y**2) * z,              # Y_4^-3
                0.75 * sqrt(5 / PI) * x * y * (7 * z**2 - 1),                         # Y_4^-2
                0.75 * sqrt(5 / (2 * PI)) * y * (7 * z**3 - 3 * z),                   # Y_4^-1
                0.1875 * sqrt(1 / PI) * (35 * z**4 - 30 * z**2 + 3),                  # Y_40
                0.75 * sqrt(5 / (2 * PI)) * x * (7 * z**3 - 3 * z),                   # Y_41
                0.375 * sqrt(5 / PI) * (x**2 - y**2) * (7 * z**2 - 1),                # Y_42
                0.75 * sqrt(35 / (2 * PI)) * x * (x**2 - 3 * y**2) * z,               # Y_43
                0.1875 * sqrt(35 / PI) * (x**2 * (x**2 - 3 * y**2) - y**2 * (3 * x**2 - y**2)),  # Y_44
            ]
